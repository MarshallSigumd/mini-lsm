...
pub struct SsTableIterator {
    table: Arc<SsTable>,
    blk_iter: BlockIterator, //用来遍历第idx个block中的所有键值对，指向
    blk_idx: usize,          //表面当前的block是目前SST中第idx个block
}
...
impl SsTableIterator {
    ...
    /// CREATE*是要先用传入的table参数，没有CREATE*的直接用&self.table
...
impl StorageIterator for SsTableIterator {
    ...
    /// Note: You may want to check if the current block iterator is valid after the move.因为可能blk_iter遍历完了
    fn next(&mut self) -> Result<()> {
        ...
        if !self.blk_iter.is_valid() {
            ...
            //如果当前blk_idx为最后一块，blk_iter已遍历完最后一块，那么不会进入if内，并且self.iter保持无效状态
        }
        Ok(())
    }
}
